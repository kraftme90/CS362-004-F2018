...
int playCouncil_room(int currentPlayer,struct gameState *state,int i,int handPos) {
    for (i = 0; i < 4; i++) {
	  drawCard(currentPlayer, state);
	}	
      //+1 Buy
...
int playFeast(int i,struct gameState *state,int currentPlayer,int* temphand,int x,int choice1) {
...
int playMine(struct gameState *state,int choice1,int choice2,int currentPlayer,int j,int i,int handPos) {
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      return playAdventurer(drawntreasure,state,currentPlayer,cardDrawn,temphand,z);
			
    case council_room:
      //+4 Cards
      return playCouncil_room(currentPlayer,state,i,handPos);
			
    case feast:
    	//gain card with cost up to 5
     	return playFeast(i,state,currentPlayer,temphand,x,choice1);
			
    case gardens:
      return -1;
			
    case mine:
		return playMine(state,choice1,choice2,currentPlayer,j,i,handPos);
