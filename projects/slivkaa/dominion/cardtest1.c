...
int main(){
    ...
    if(actual == expected){
            printf("TEST 6: PASS smithy card was discarded after play (discarded?= %d, expected = %d)\n", actual, expected);
            numPasses++;
    }
    else{
        printf("TEST 6: FAIL smithy card wasn't discarded after play (discarded? = %d, expected = %d)\n", actual, expected);
    }
