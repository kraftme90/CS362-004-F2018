 ...
 *
 * <p>
 * Domain names are evaluated according to the standards
 * <a href="http://www.ietf.org/rfc/rfc1034.txt">RFC1034</a>, section 3, and
 * <a href="http://www.ietf.org/rfc/rfc1123.txt">RFC1123</a>, section 2.1. No
 * accommodation is provided for the specialized needs of other applications; if
...
public class DomainValidator implements Serializable {
	...
	// Note that the regex currently requires both a domain label and a top level
	// label, whereas
	// the RFC does not. This is because the regex is used to detect if a TLD is
	// present.
	// If the match fails, input is checked against DOMAIN_LABEL_REGEX
	// (hostnameRegex)
	...
	/**
	 * Singleton instance of this validator, which doesn't consider local addresses
	 * as valid.
	 */
	private static final DomainValidator DOMAIN_VALIDATOR = new DomainValidator(false);

	/**
	 * Singleton instance of this validator, which does consider local addresses
	 * valid.
	 */
	...
	/**
	 * Returns the singleton instance of this validator. It will not consider local
	 * addresses as valid.
	 * 
	...
	/**
	 * Returns true if the specified <code>String</code> parses as a valid domain
	...
	/**
	 * Returns true if the specified <code>String</code> matches any IANA-defined
	 * top-level domain. Leading dots are ignored if present. The search is
	 * case-insensitive.
	 * 
	...
	/**
	 * Returns true if the specified <code>String</code> matches any IANA-defined
	 * country code top-level domain. Leading dots are ignored if present. The
	 * search is case-insensitive.
	 * 
	...
	/**
	 * Returns true if the specified <code>String</code> matches any widely used
	...
	// the text file. These all have the "Not assigned" in the "Sponsoring
	// Organisation" column
	// For example (as of 2015-01-02):
	// .bl country-code Not assigned
	// .um country-code Not assigned

	// WARNING: this array MUST be sorted, otherwise it cannot be searched reliably
	// using binary search
	private static final String[] INFRASTRUCTURE_TLDS = new String[] { "arpa", // internet infrastructure
	};
...
//        "iinet", // iinet Connect West Pty. Ltd. (Retired)
			...
			"melbourne", // melbourne The Crown in right of the State of Victoria, represented by its
			...
			"moscow", // moscow Foundation for Assistance for Internet Technologies and Infrastructure
						// Development (FAITID)
...
//        "mutuelle", // mutuelle Fédération Nationale de la Mutualité Française (Retired)
			...
			"tatar", // tatar Limited Liability Company &quot;Coordination Center of Regional Domain
			...
			"xn--55qx5d", // 公司 Computer Network Information Center of Chinese Academy of Sciences （China
			...
			"xn--80adxhks", // москва Foundation for Assistance for Internet Technologies and Infrastructure
							// Development (FAITID)
			"xn--80aqecdr1a", // католик Pontificium Consilium de Comunicationibus Socialibus (PCCS)
								// (Pontifical Council for Social Communication)
			...
			"xn--io0a7i", // 网络 Computer Network Information Center of Chinese Academy of Sciences （China
							// Internet Network Information Center）
			...
			"xn--tiq49xqyj", // 天主教 Pontificium Consilium de Comunicationibus Socialibus (PCCS) (Pontifical
...
//        "tp",                 // East Timor (Retired)
	...
	// WARNING: this array MUST be sorted, otherwise it cannot be searched reliably
	// using binary search
	private static final String[] LOCAL_TLDS = new String[] { "localdomain", // Also widely used as
	...
	/*
	 * This field is used to detect whether the getInstance has been called. After
	 * this, the method updateTLDOverride is not allowed to be called. This field
	 * does not need to be volatile since it is only accessed from synchronized
	 * methods.
	 */
	private static boolean inUse = false;

	/*
	 * These arrays are mutable, but they don't need to be volatile. They can only
	 * be updated by the updateTLDOverride method, and any readers must get an
	 * instance using the getInstance methods which are all (now) synchronised.
	 */
	// WARNING: this array MUST be sorted, otherwise it cannot be searched reliably
	// using binary search
	...
	public enum ArrayType {
		/**
		 * Update (or get a copy of) the GENERIC_TLDS_PLUS table containing additonal
		 * generic TLDs
		 */
		GENERIC_PLUS,
		/**
		 * Update (or get a copy of) the GENERIC_TLDS_MINUS table containing deleted
		 * generic TLDs
		 */
		GENERIC_MINUS,
		/**
		 * Update (or get a copy of) the COUNTRY_CODE_TLDS_PLUS table containing
		 * additonal country code TLDs
		 */
		COUNTRY_CODE_PLUS,
		/**
		 * Update (or get a copy of) the COUNTRY_CODE_TLDS_MINUS table containing
		 * deleted country code TLDs
		 */
		COUNTRY_CODE_MINUS,
	 ...
	 * startup, before any instances are accessed using getInstance.
	 * <p>
	 * For example:
	 * <p>
	 * {@code DomainValidator.updateTLDOverride(ArrayType.GENERIC_PLUS, new
	 * String[]{"apache"})}
	 * <p>
	 * To clear an override array, provide an empty array.
	 *
	 * @param table the table to update, see {@link DomainValidator.ArrayType} Must
	 ...
	 * @throws IllegalArgumentException if the table type is unexpected (should not
	 *                                  happen)
	 * @since 1.5.1
	 */
