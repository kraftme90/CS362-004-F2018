...
public class UrlValidator implements Serializable {
	...
	/**
	 * Allow local URLs, such as http://localhost/ or http://machine/ . This enables
	 * a broad-brush check, for complex local machine name validation requirements
	 * you should create your validator with a {@link RegexValidator} instead
	 * ({@link #UrlValidator(RegexValidator, long)})
	 */
	...
	private static final String IPV6_REGEX = "[0-9a-fA-F:]+"; // do this as separate match because : could cause
																// ambiguity with port prefix
	...
	// since neither ':' nor '@' are allowed chars, we don't need to use non-greedy
	// matching
	private static final String USERINFO_FIELD_REGEX = USERINFO_CHARS_REGEX + "+" + // At least one character for the
																					// name
			"(?::" + USERINFO_CHARS_REGEX + "*)?@"; // colon and password may be absent
	private static final String AUTHORITY_REGEX = "(?:\\[(" + IPV6_REGEX + ")\\]|(?:(?:" + USERINFO_FIELD_REGEX + ")?(["
	...
	/**
	 * Regular expressions used to manually validate authorities if IANA domain name
	 ...
	 * 
	 * @param options The options should be set using the public constants declared
	 ...
	 * Behavior of validation is modified by passing in options:
	 * 
	 * @param schemes The set of valid schemes. Ignored if the ALLOW_ALL_SCHEMES
	 *                option is set.
	 * @param options The options should be set using the public constants declared
	 *                in this class. To set multiple options you simply add them
	 *                together. For example, ALLOW_2_SLASHES + NO_FRAGMENTS enables
	 ...
	 * Initialize a UrlValidator with the given validation options.
	 * 
	 * @param authorityValidator Regular expression validator used to validate the
	 *                           authority part This allows the user to override the
	 *                           standard set of domains.
	 * @param options            Validation options. Set using the public constants
	 *                           of this class. To set multiple options, simply add
	 *                           them together:
	 *                           <p>
	 ...
	 * </p>
	 *
	 * Note that the method calls #isValidAuthority() which checks that the domain
	 * is valid.
	 *
	...
	/**
	 * Validate scheme. If schemes[] was initialized to a non null, then only those
	 * schemes are allowed. Otherwise the default schemes are "http", "https",
	...
	protected boolean isValidScheme(String scheme) {
		...
		// TODO could be removed if external schemes were checked in the ctor before
		// being stored
	 ...
	 * considered invalid. Note: this implementation validates the domain unless a
	 * RegexValidator was provided. If a RegexValidator was supplied and it matches,
	 * then the authority is regarded as valid with no further checks, otherwise the
	 * method checks against the AUTHORITY_PATTERN and the DomainValidator
	 * (ALLOW_LOCAL_URLS)
	 * 
	...
	protected boolean isValidAuthority(String authority) {
		...
		// We have to process IPV6 separately because that is parsed in a different
		// group
	 ...
	 * this tests whether the combination of flags is on.
	 *
	...
	/**
	 * Tests whether the given flag is off. If the flag is not a power of 2 (ie. 3)
	 * this tests whether the combination of flags is off.
	 *
	 * @param flag Flag value to check.
	 *
