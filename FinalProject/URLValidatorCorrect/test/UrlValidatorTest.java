...
public class UrlValidatorTest extends TestCase {
	...
	private final boolean printIndex = false;// print index that indicates current scheme,host,port,path, query test
												// were using.
...
//        testIsValid(testUrlPartsOptions, options);
	 ...
	 * <scheme>://<authority><path>?<query> except that the port number is broken
	 * out of authority to increase the number of permutations. A complete URL is
	 * composed of a scheme+authority+port+path+query, all of which must be
	 * individually valid for the entire URL to be considered valid.
	 */
	...
	ResultPair[] testUrlPort = { new ResultPair(":80", true), new ResultPair(":65535", true),
			...
			new ResultPair(":65636", false), new ResultPair(":65a", false) };
	...
	ResultPair[] testUrlPathOptions = { new ResultPair("/test1", true), new ResultPair("/t123", true),
			...
			new ResultPair("/test1/file", true), new ResultPair("/t123/file", true), new ResultPair("/$23/file", true),
			new ResultPair("/../file", false), new ResultPair("/..//file", false), new ResultPair("/test1//file", true),
